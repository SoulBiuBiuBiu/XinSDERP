...
@Controller(value = "mjlbAction")
@Scope(value = "PROTOTYPE")
@Namespace("/mjlb")
@ParentPackage("XinSD")
public class MjlbAction extends BaseAction implements ModelDriven<Mjlb> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private MjlbService service;
	private Mjlb mjlb;
	private int page;
	private int rows;
	private String sort;
	...
	@Autowired
	MjlbAction(MjlbService mjlbService) {
		this.service = mjlbService;
	}

	final DetachedCriteria getDetachedCriteria() {
		DetachedCriteria dc = DetachedCriteria.forClass(Mjlb.class);
	...
	// 查 查询分页列表数据
	/**
	 * Mjlb_getPageBean 查询分页列表数据 parame;int currentPage;int rows;qx对象 {Integer
	...
	public String getPageBean() {
		...
		List<Mjlb> list = (List<Mjlb>) pageBean.getList();

		for (int i = 0; i < list.size(); i++) {
			StringBuffer buffer = new StringBuffer();
			buffer.append("<a href='#' onclick=\"UpdateTabs('order','/XinSDERP/mjlb/getByModel?mjlb.id="
					+ list.get(i).getId().toString() + "','','iconfont icon-edit');\">编辑</a>&nbsp;&nbsp;");
			buffer.append("<a href='#' onclick=\"Data_Remove('/XinSDERP/mjlb/deleteById?mjlb.id="
	...
	public String getByModel() {
		Mjlb model = service.getByModel(mjlb);
		HttpServletRequest request = ServletActionContext.getRequest();
		request.setAttribute("mjlb", model);
		return "success";
	}
	...
	public MjlbService getService() {
		return service;
	}

	public void setService(MjlbService service) {
	...
	public void setModel(Mjlb mjlb) {
		this.mjlb = mjlb;
	}

	public void setMjlbService(MjlbService service) {
		this.service = service;
	}

	@Override
	public Mjlb getModel() {
		return mjlb;
	}
}
