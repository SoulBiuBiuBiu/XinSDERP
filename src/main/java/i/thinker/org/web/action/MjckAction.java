...
@Controller(value = "mjckAction")
@Scope(value = "PROTOTYPE")
@Namespace("/mjck")
@ParentPackage("XinSD")
public class MjckAction extends BaseAction implements ModelDriven<Mjck> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private MjckService service;
	private Mjck mjck;
	private int page;
	private int rows;
	private String sort;
	...
	@Autowired
	MjckAction(MjckService mjckService) {
		this.service = mjckService;
	}

	final DetachedCriteria getDetachedCriteria() {
		DetachedCriteria dc = DetachedCriteria.forClass(Mjck.class);
	...
	// 查 查询分页列表数据
	/**
	 * Mjck_getPageBean 查询分页列表数据 parame;int currentPage;int rows;qx对象 {Integer
	...
	public String getPageBean() {
		...
		List<Mjck> list = (List<Mjck>) pageBean.getList();

		for (int i = 0; i < list.size(); i++) {
			StringBuffer buffer = new StringBuffer();
			buffer.append("<a href='#' onclick=\"UpdateTabs('order','/XinSDERP/mjck/getByModel?mjck.id="
					+ list.get(i).getId().toString() + "','','iconfont icon-edit');\">编辑</a>&nbsp;&nbsp;");
			buffer.append("<a href='#' onclick=\"Data_Remove('/XinSDERP/mjck/deleteById?mjck.id="
	...
	public String getByModel() {
		Mjck model = service.getByModel(mjck);
		HttpServletRequest request = ServletActionContext.getRequest();
		request.setAttribute("mjck", model);
		return "success";
	}
	...
	public void setMjck(Mjck mjck) {
		this.mjck = mjck;
	}

	public MjckService getService() {
		return service;
	}

	public void setService(MjckService service) {
	...
	public void setModel(Mjck mjck) {
		this.mjck = mjck;
	}

	public void setMjckService(MjckService service) {
		this.service = service;
	}

	@Override
	public Mjck getModel() {
		return mjck;
	}
}
