...
@Controller(value = "lcAction")
@Scope(value = "PROTOTYPE")
@Namespace("/lc")
@ParentPackage("XinSD")
public class LcAction extends BaseAction implements ModelDriven<Lc> {
	...
	@Autowired
	LcAction(LcService lcService) {
		this.service = lcService;
	}

	final DetachedCriteria getDetachedCriteria() {
		DetachedCriteria dc = DetachedCriteria.forClass(Lc.class);
	...
	// 查 查询分页列表数据
	/**
	 * Lc_getPageBean 查询分页列表数据 parame;int currentPage;int rows;qx对象 {Integer
	...
	public String getPageBean() {
		...
		List<Lc> list = (List<Lc>) pageBean.getList();

		for (int i = 0; i < list.size(); i++) {
			StringBuffer buffer = new StringBuffer();
			buffer.append("<a href='#' onclick=\"UpdateTabs('order','/XinSDERP/lc/getByModel?lc.id="
					+ list.get(i).getId().toString() + "','','iconfont icon-edit');\">编辑</a>&nbsp;&nbsp;");
			buffer.append("<a href='#' onclick=\"Data_Remove('/XinSDERP/lc/deleteById?lc.id="
	...
	public String getByModel() {
		Lc model = service.getByModel(lc);
		HttpServletRequest request = ServletActionContext.getRequest();
		request.setAttribute("lc", model);
		return "success";
	}
	...
	public void setService(LcService service) {
	...
	public void setModel(Lc lc) {
		this.lc = lc;
	}

	public void setLcService(LcService service) {
		this.service = service;
	}
