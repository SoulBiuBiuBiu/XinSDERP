...
@Controller(value = "cpckAction")
@Scope(value = "PROTOTYPE")
@Namespace("/cpck")
@ParentPackage("XinSD")
public class CpckAction extends BaseAction implements ModelDriven<Cpck> {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	private CpckService service;
	private Cpck cpck;
	private int page;
	private int rows;
	private String sort;
	...
	@Autowired
	CpckAction(CpckService cpckService) {
		this.service = cpckService;
	}

	final DetachedCriteria getDetachedCriteria() {
		DetachedCriteria dc = DetachedCriteria.forClass(Cpck.class);
	...
	// 查 查询分页列表数据
	/**
	 * Cpck_getPageBean 查询分页列表数据 parame;int currentPage;int rows;qx对象 {Integer
	...
	public String getPageBean() {
		...
		List<Cpck> list = (List<Cpck>) pageBean.getList();

		for (int i = 0; i < list.size(); i++) {
			StringBuffer buffer = new StringBuffer();
			buffer.append("<a href='#' onclick=\"UpdateTabs('order','/XinSDERP/cpck/getByModel?cpck.id="
					+ list.get(i).getId().toString() + "','','iconfont icon-edit');\">编辑</a>&nbsp;&nbsp;");
			buffer.append("<a href='#' onclick=\"Data_Remove('/XinSDERP/cpck/deleteById?cpck.id="
	...
	public String getByModel() {
		Cpck model = service.getByModel(cpck);
		HttpServletRequest request = ServletActionContext.getRequest();
		request.setAttribute("cpck", model);
		return "success";
	}
	...
	public void setCpck(Cpck cpck) {
		this.cpck = cpck;
	}

	public CpckService getService() {
		return service;
	}

	public void setService(CpckService service) {
	...
	public void setModel(Cpck cpck) {
		this.cpck = cpck;
	}

	public void setCpckService(CpckService service) {
		this.service = service;
	}

	@Override
	public Cpck getModel() {
		return cpck;
	}
}
